"""
videoloader from https://github.com/sihyun-yu/PVDM/blob/main/tools/dataloader.py
"""
...
def get_loaders(rank, data_location, imgstr, resolution, timesteps, skip, batch_size=1, test_batch_size=1, n_gpus=1, seed=42,  cond=False, num_workers=4):
    """
    ...
    trainloader = DataLoader(trainset, shuffle = True, batch_size=batch_size // n_gpus, pin_memory=False, num_workers=num_workers)
    
    #testset_sampler = InfiniteSampler(testset, num_replicas=n_gpus, rank=rank, seed=seed)
    testloader = DataLoader(testset, shuffle=False, batch_size=test_batch_size, pin_memory=False, num_workers=4, prefetch_factor=2)
